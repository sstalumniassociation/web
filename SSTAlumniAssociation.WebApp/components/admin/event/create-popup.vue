...
const { mutate: createEventMutate, isPending: createEventIsPending } = useAdminCreateEventMutation()
...
<template>
  <Dialog v-model:visible="visible" modal class="w-full mx-4 md:w-auto md:min-w-xl">
    ...
    <form class="flex flex-col space-y-3 mt-2" @submit="createEvent">
      <InputText
        v-model="name" autofocus placeholder="Name (example: SST Homecoming)" class="w-full" v-bind="nameProps"
        :invalid="!!errors.name"
      />
      <small class="p-error">{{ errors.name }}</small>

      <Textarea
        v-model="description" v-bind="descriptionProps" rows="5"
        placeholder="Description (example: Come back to 1 Technology Drive for our inaugural homecoming!)"
        class="w-full resize-y" :invalid="!!errors.description"
      />
      ...
      <div class="flex flex-gap-3">
        ...
        <Calendar
          v-model="startDateTime" time-only show-time hour-format="12" placeholder="Time"
      ...
      <div class="flex flex-gap-3">
        ...
        <Calendar
          v-model="endDateTime" time-only show-time hour-format="12" placeholder="End time"
      ...
      <InputText
        v-bind="locationProps" v-model="location"
        placeholder="Location (example: 1 Technology Drive, Singapore)" class="w-full" :invalid="!!errors.location"
      />
      <small class="p-error">{{ errors.location }}</small>

      <InputText
        v-bind="badgeImageProps" v-model="badgeImage" type="url" :invalid="!!errors.badgeImage"
